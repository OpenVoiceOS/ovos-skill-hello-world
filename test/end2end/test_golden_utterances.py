...
derived entirely from this skill's own intent templates: ``hello_world_intent``,
``thank_you_intent``, ``greetings`` and ``how_are_you`` (all Padatious/Padacioso,
...
def test_golden_utterance(minicroft, row):
    ...
    utterances = [m.data.get("utterance") for m in spoken]
    valid_lines = _dialog_lines(_INTENT_DIALOG[row["intent_label"]])
    # Every spoken line, not only the first: a handler that speaks the
    # right line and then a wrong one must fail too.
    assert all(u in valid_lines for u in utterances), (
        f"{row['utterance']!r}: spoke {utterances!r}, not all in the lines of "
